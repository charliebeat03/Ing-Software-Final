...
class HistorialWidget(QWidget):
    ...
    def conectar_señales(self):
        ...
        try:
            if hasattr(self, 'historial_manager') and hasattr(self.historial_manager, 'datos_actualizados'):
                try:
                    self.historial_manager.datos_actualizados.connect(self.actualizar_ui)
                except Exception:
                    # Puede fallar si el objeto no es un QObject válido; no bloquear la UI
                    pass
        except Exception as e:
            print(f"❌ Error conectando señales historial: {e}")
    ...
    def actualizar_ui(self):
        ...
        try:
            vista = self.combo_vista.currentData() if hasattr(self, 'combo_vista') else 'dia'
            if vista == 'dia':
                self.cargar_datos_dia()
            else:
                self.cargar_datos_mes()
        except Exception as e:
            print(f"❌ Error en actualizar_ui: {e}")
