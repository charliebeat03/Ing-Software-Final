...
class ProduccionManager:
    ...
    def agregar_produccion_a_ventas(self, produccion_id):
        ...
        try:
            ...
            # Agregar a ventas: VentasManager.registrar_venta devuelve el total de la venta
            try:
                total_venta = ventas_manager.registrar_venta(producto_id, cantidad)
                venta_id = None
                print(f"✅ Producción agregada a ventas: total_venta={total_venta}")
            except Exception as e:
                print(f"⚠️ No se pudo registrar venta automática: {e}")
                venta_id = None
                total_venta = cantidad * precio if precio is not None else 0
