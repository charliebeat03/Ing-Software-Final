...
# Integración y correcciones menores

import traceback
from datetime import datetime, date, timedelta
from database import db  # Usamos el singleton de database.py
...
class CierreDiarioManager:
    def __init__(self, db_path=None):
        try:
            ...
            self.db = db
            print("✅ CierreDiarioManager inicializado con Database")
    ...
    def obtener_resumen_ventas_dia(self, fecha=None):
        ...
        try:
            if fecha is None:
                ...
                fecha_actual = fecha if hasattr(fecha, 'strftime') else date.fromisoformat(str(fecha))
    ...
    def obtener_excedentes_dia(self, fecha=None):
        ...
        try:
            ...
            for prod in produccion:
                ...
                try:
                    producido = float(prod.get('producido', 0))
                except Exception:
                    producido = 0
                ...
                if excedente > 0:
                    ...
                    valor_excedente = excedente * precio
    ...
    def registrar_ajuste_ingrediente(self, ingrediente_id, cantidad_usada, cantidad_pedida, fecha):
        ...
        try:
            ...
            # Permitir diferencia == 0 (no hay devolución). Error sólo si negativa.
            if diferencia < 0:
                return {
                    'success': False,
                    'error': 'La cantidad usada no puede ser mayor a la pedida'
                }

            # 1. Actualizar inventario (sumar diferencia)
            update_query = '''
            ...
            self.db.execute_query(update_query, (diferencia, ingrediente_id), commit=True)
            ...
            # Usamos execute_insert para obtener id si es necesario
            try:
                ...
                # Fallback a execute_query con commit
    ...
    def cerrar_dia(self, ajustes, fecha=None):
        ...
        try:
            fecha_actual = fecha if fecha is not None and hasattr(fecha, 'strftime') else date.today()
            fecha_str = fecha_actual.strftime('%Y-%m-%d')
            ...
            for ajuste in ajustes:
                ...
                if resultado.get('success'):
                    resultados.append({
                        ...
                        'diferencia_devuelta': resultado.get('diferencia_devuelta', 0)
                    })
                else:
            ...
            if resultados:
                ...
                self.db.execute_query(cierre_query, (fecha_str, notas), commit=True)
